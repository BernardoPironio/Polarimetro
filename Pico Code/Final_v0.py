...
# STEP fijo ya no se usa directamente en el timer.
# Ahora se controla con motor_step[0].
STEP_INICIAL = 4
STEP_FINAL   = 7
...
# Compartido entre núcleos
state      = array('I', [0])              # state[0] = phase_idx
running    = array('I', [1])              # running[0] = 1 mientras corre
motor_step = array('I', [STEP_INICIAL])   # step dinámico del motor
...
def motor_tick(timer, _u=pwm_u, _v=pwm_v, _w=pwm_w,
               ...
               _st=state, _step_arr=motor_step, _sz=TABLE_SIZE):
    ...
    # IMPORTANTE:
    # _step_arr es el array motor_step.
    # El valor real está en _step_arr[0].
    _st[0] = (idx + _step_arr[0]) % _sz
...
try:
    ...
    # RAMPA DE ARRANQUE
    # =====================================================
    print("Rampa de arranque...")

    for s in range(STEP_INICIAL, STEP_FINAL + 1):
        motor_step[0] = s
        print("STEP =", s)
        time.sleep_ms(400)

    print("Rampa lista. STEP final =", motor_step[0])

    # =====================================================
    # AHORA ARRANCA LA MEDICIÓN
    # =====================================================
    idx_adc = 0
    count = 0
    md_prev = 0
    ...
    while idx_adc < N:
        ...
        # Por defecto, en esta fila no hubo pulso
        iman_pulse[idx_adc] = 0
        periodo_us[idx_adc] = 0

        # Flanco ascendente: pasó el imán
        if md == 1 and md_prev == 0:
            ...
            if count == 0:
                ...
                periodo_us[idx_adc] = time.ticks_diff(now_us, t_antes_us)
    ...
    print("Muestras totales:", idx_adc)
