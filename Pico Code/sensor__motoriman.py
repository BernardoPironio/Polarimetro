...
import time
import gc  # <-- IMPORTAR EL MÓDULO GC
...
# -----------------------------
# PREPARAR MEMORIA (evita GC durante el movimiento)
# -----------------------------
print("Preparando memoria...")
gc.collect()           # Limpiar memoria ahora
gc.disable()           # <--- DESACTIVAR GARBAGE COLLECTOR
print("GC desactivado - el motor no debería golpear")

# Reservar espacio para variables temporales (opcional)
temp_vars = [0.0] * 10  # Reserva memoria de antemano
...
try:
    ...
    while True:
        ...
        # LIMITAR dt MÁXIMO (evita saltos grandes si hay micro-pausas)
        if dt > 0.01:  # Máximo 10ms
            dt = 0.01

        # avanzar ángulo eléctrico
        theta += VELOCIDAD * dt
        
        # Mantener theta en rango (evita overflow)
        if theta > 2 * pi:
            theta -= 2 * pi
        ...
        # Pequeña pausa para estabilidad (opcional)
        # time.sleep_us(50)  # Descomentar si es necesario
    ...
    # Reactivar GC antes de apagar (por si acaso)
    gc.enable()
    ...
    print(f"Pulsos detectados: {count}")
    print("GC reactivado")