    ...
    if interrumpido:
        ...
        try:
            with open("datos_motor.csv", "w") as f:
                f.write("periodo_ms,time_us,adc_raw\n")

                n_adc = min(idx_adc, N)
                n_periodos = min(count, 1000)
                n_filas = max(n_adc, n_periodos)

                for i in range(n_filas):
                    p = periodos[i] if i < n_periodos else ""
                    t = times[i] if i < n_adc else ""
                    adc = data_adc[i] if i < n_adc else ""
                    f.write("{},{},{}\n".format(p, t, adc))

            print("Guardado en la Pico: datos_motor.csv")
            print("Filas ADC:", n_adc, "Periodos:", n_periodos)
